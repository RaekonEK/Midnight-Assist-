...
import com.one_studio.mixin.client.MinecraftClientAccessor
...
object MidnightAssisitClient : ClientModInitializer {
    ...
    private fun findTarget(client: MinecraftClient, range: Double): Entity? {
        ...
        for (entity in entities) {
            // استهداف الوحوش واللاعبين الآخرين (إذا لم يكونوا في نفس الفريق مثلاً، لكن هنا سنبقيها عامة)
            if (entity is MobEntity || entity is PlayerEntity) {
                if (entity == player || entity is ArmorStandEntity) continue
    ...
    private fun snapToEntity(player: PlayerEntity, entity: Entity) {
        ...
        val dy = targetPos.y - player.eyeY
